...
def process_task(task: Task, weights: np.array, save_dir: str):
    ...
    for i in range(task.count_examples):
        ...
        output_data = datapoints_from_image(i, output_image)

        random.Random(i + 1000).shuffle(input_data)
    ...
    if True:
        ...
        input_data = datapoints_from_image(pair_id, input_image)
        output_image = task.test_output(0)
        output_data = datapoints_from_image(pair_id, output_image)

        random.Random(i + 3000).shuffle(input_data)
        random.Random(i + 4000).shuffle(output_data)
        sampled_data = sample_data(input_data, output_data, rng)

        # print(f"TEST input: {len(input_data)} output: {len(output_data)} samples: {len(sampled_data)}")
        input_target_pairs_one_test += sampled_data
