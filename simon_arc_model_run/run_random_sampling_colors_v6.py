    ...
    for input_data_samples, target_data_samples in input_target_pairs:
        ...
        for y in range(n):
            for x in range(n):
                ...
                # xs_item.append(1 if input_value == 0 else 0)

                position_values = [
                    target_x, target_y, 
                    target_x_rev, target_y_rev,
                    input_x, input_y,
                    input_x_rev, input_y_rev,
                ]
                for pv in position_values:
                    xs_item.append(pv & 1)
                    xs_item.append((pv >> 1) & 1)
                    xs_item.append((pv >> 2) & 1)
                    xs_item.append((pv >> 3) & 1)
                    # xs_item.append((pv >> 4) & 1)
