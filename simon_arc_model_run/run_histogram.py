...
from simon_arc_lab.color_constraint_analyzer import ColorConstraintAnalyzer, Metric, ColorConstraintAnalyzerRunner
...
runner = ColorConstraintAnalyzerRunner()
...
for index, (groupname, path_to_task_dir) in enumerate(groupname_pathtotaskdir_list):
    ...
    for task in taskset.tasks:
        if task.metadata_task_id in invalid_task_id_list:
            continue
        runner.measure_task(task)

    end_time = time.time()
    elapsed_time = end_time - start_time
    total_elapsed_time += elapsed_time

print(f"\nIssues: {runner.count_issue}, puzzles where the transformation couldn't be identified.")

print(f"\nCorrect:")
sorted_counters = sorted(runner.count_correct.items(), key=lambda x: (-x[1], x[0].name))
...
sorted_counters = sorted(runner.count_incorrect.items(), key=lambda x: (-x[1], x[0].name))
for key, count in sorted_counters:
    s = key.format_with_value(count)
    print(f"  {s}")

print(f"\nCorrect subitems:")
sorted_counters = sorted(runner.count_correct_subitem.items(), key=lambda x: (-x[1], x[0].name))
for key, count in sorted_counters:
    s = key.format_with_value(count)
    print(f"  {s}")

print(f"\nIncorrect subitems, where the check was triggered, but not satisfied, a false positive:")
sorted_counters = sorted(runner.count_incorrect_subitem.items(), key=lambda x: (-x[1], x[0].name))
for key, count in sorted_counters:
    s = key.format_with_value(count)
    print(f"  {s}")
