...
def calc_color_to_polar(color: int, offset: float) -> tuple[float, float]:
    angle = (color + offset) * 2 * np.pi / 10
    return (np.cos(angle), np.sin(angle))

def calc_polar_to_color(polar: tuple[float, float], offset: float) -> int:
    x, y = polar
    angle = np.arctan2(y, x)
    color = int((10 * angle) / (2 * np.pi) - offset)
    if color < 0:
        color += 10
    return color

def xs_ys_from_input_target_pairs(input_target_pairs: list, color_obfuscation_seed: int) -> tuple[list, list]:
    xs = []
    ys = []
    extra = []
    inner_index = -1
    for input_data_samples, target_data_samples in input_target_pairs:
        ...
        for y in range(n):
            for x in range(n):
                ...
                # color_offset = (inner_index * color_obfuscation_seed * 3373.73) % 10.0
                # color_offset = (inner_index * color_obfuscation_seed * 3373.73) % 3.0
                pixel_position = (30 * 30) * target_pair_index + (target_y * target_width) + target_x
                # color_offset = (pixel_position * 3373.73) % 3.0
                color_offset = (pixel_position * 3373.73) % 0.25
                color_tuple = calc_color_to_polar(target_value, color_offset)
                ys_item = color_tuple
...
def process_task(task: Task, weights: np.array, save_dir: str):
    ...
    xs, ys, extra = xs_ys_from_input_target_pairs(input_target_pairs, 3838383)
    # clf = DecisionTreeClassifier(random_state=42, max_depth=15)
    clf = tree.DecisionTreeRegressor(random_state=42, max_depth=15)
    clf.fit(xs, ys)

    xs2, ys2, extra2 = xs_ys_from_input_target_pairs(input_target_pairs_one_test, 15833)
    ...
    # Convert from a list of np.float64 to a list of regular Python floats
    ys2converted = [(float(a), float(b)) for a, b in ys2]

    # print(f"predicted_values: {predicted_values}")
    # print(f"ys2: {ys2}")
    # print(f"ys2converted: {ys2converted}")
    ...
    for i in range(len(predicted_values)):
        diff_sum = 0.0
        for j in range(2):
            diff = predicted_values[i][j] - ys2converted[i][j]
            diff_sum += diff * diff
        if diff_sum < 0.01:
    ...
    if True:
        ...
        for i in range(len(predicted_values)):
            ...
            color_polar = predicted_values[i]
            color = calc_polar_to_color(color_polar, color_offset)
