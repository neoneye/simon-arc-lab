...
def xs_ys_from_input_target_pairs(input_target_pairs: list, classifier_color: int) -> tuple[list, list]:
    xs = []
    ys = []
    extra = []
    for input_data_samples, target_data_samples in input_target_pairs:
        ...
        for y in range(n):
            for x in range(n):
                ...
                ys_item = target_value == classifier_color
...
def process_task(task: Task, weights: np.array, save_dir: str):
    ...
    for color_index in range(10):
        xs, ys, extra = xs_ys_from_input_target_pairs(input_target_pairs, color_index)
        clf = DecisionTreeClassifier(random_state=42, max_depth=15)
        clf.fit(xs, ys)

        xs2, ys2, extra2 = xs_ys_from_input_target_pairs(input_target_pairs_one_test, color_index)
        ...
        dict_xs[color_index] = xs2
        dict_pred[color_index] = predicted_values
        dict_extra[color_index] = extra2
        ...
        accumulated_incorrect += pred_count_incorrect
        ...
        print(f"task: {task.metadata_task_id} color: {color_index} correct: {pred_count_correct} incorrect: {pred_count_incorrect}")

    pred_is_correct = accumulated_incorrect == 0
    ...
    if True:
        ...
        for color_index in range(10):
            xs2 = dict_xs[color_index]
            predicted_values = dict_pred[color_index]
            extra2 = dict_extra[color_index]
            ...
            for i in range(len(predicted_values)):
                ...
                if color > 0:
                    color_count_image[color_index][target_y, target_x] += 1
