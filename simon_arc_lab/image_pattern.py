...
def image_pattern_lines_vertical(width: int, height: int, line_size: int, offsetx: int, colors: list[int]) -> np.array:
    """
    Create a pattern with repeating vertical lines.
...
def image_pattern_lines_slope_advanced(width: int, height: int, dx: int, dy: int, square_size: int, offsetx: int, offsety: int, colors: list[int]) -> np.array:
    ...
    :param width: The width of the image
    :param height: The height of the image
    :param dx: Move dx pixels in x direction, can be negative/positive/zero
    :param dy: Move dy pixels in y direction, can be negative/positive/zero
    ...
    if dx != 0 and dy != 0:
        # Normalize the slope
        gcd = np.gcd(dx, dy)
        step_x = dx // gcd # Can be negative/positive, but not zero
        step_y = dy // gcd # Can be negative/positive, but not zero
    if dx == 0:
        step_x = 0
